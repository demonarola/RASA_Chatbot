...
$(document).ready(function () {
	...
	var bot =
		...
		'<div class="row">' +
		'<div id="Smallchat">' +
		'<div class="Layout Layout-open Layout-right" style="background-color: #3F51B5;color: rgb(255, 255, 255);opacity: 5;border-radius: 10px;">' +
		'<div class="Messenger_messenger">' +
		'<div class="Messenger_header" style="background-color: rgb(22, 46, 98); color: rgb(255, 255, 255);">' +
		'<h4 class="Messenger_prompt">Pizza Bot</h4><span class="chat_close_icon"><i class="fa fa-window-close" aria-hidden="true"></i></span>' +
		'</div>' +
		'<div class="Messenger_content">' +
		'<div class="Messages">' +
		'<div id="result_div" class="Messages_list" style="color:black">' +
		'</div>' +
		'<div>' +
		'<div class="Input Input-blank">' +
		'<textarea id="chat-input" class="Input_field" placeholder="Send a message..." style="height: 20px;"></textarea>' +
	...
	function send(text, uniq) {
		$.ajax({
			...
			data: JSON.stringify({
				"sender": uniq, //add the user ID here
				"message": text
			}),
	...
	function addSuggestion(textToAdd) {
		setTimeout(function () {
			...
			for (i = 0; i < suggLength; i++) {
				// $('<p class="sugg-options">' + suggestions[i].title + '</p>').appendTo('.suggestion');
				$('<button style="background-color:rgb(22, 46, 98);color:white;border: 2px solid white; border-radius: 10px;" value=' + suggestions[i].payload + '>' + suggestions[i].title + '</button>').appendTo('.suggestion');
			}
			scrollToBottomOfResults();
		}, 1000);
	}
